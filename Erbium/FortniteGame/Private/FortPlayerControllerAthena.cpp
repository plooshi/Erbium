...
#include <d3d11.h>  // needed for the dump commands
#include <sstream>
#include <fstream>
...
void AFortPlayerControllerAthena::ServerCheat(UObject* Context, FFrame& Stack)
{
	...
	else
	{
		...
		// added by ducki67
			// just added Just for my self kinda who uses commands all the time
		else if (command == "dumpplaylits")
		{
			std::stringstream ss;
			...
			of << ss.str();
			of.close();
			PlayerController->ClientMessage(FString(L"Dumped playlists to DumpedPlaylists.txt"), FName(), 1.f);
			}
		...
		else if (command == "dumpitems")
		{
			...
			of << ss.str();
			of.close();
			PlayerController->ClientMessage(FString(L"Dumped items to DumpedItems.txt"), FName(), 1.f);
			}
