...
void UFortKismetLibrary::Hook()
{
	...
	if (GiveItemToInventoryOwnerFn)
		for (auto& Param : GiveItemToInventoryOwnerFn->GetParamsNamed())
		{
	...
	if (K2_RemoveItemFromPlayerFn)
		for (auto& Param : K2_RemoveItemFromPlayerFn->GetParamsNamed())
		{
			if (Param.Name == "ItemVariantGuid")
			{
	...
	if (K2_SpawnPickupInWorldFn)
		for (auto& Param : K2_SpawnPickupInWorldFn->GetParamsNamed())
		{
	...
	if (PickLootDropsFn)
		for (auto& Param : PickLootDropsFn->GetParamsNamed())
		{
