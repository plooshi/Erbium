...
FLateGameItem LateGame::GetShotgun()
{
    ...
    if (Shotguns.size() == 0)
    {
        ...
        else if (VersionInfo.FortniteVersion <= 18.40)
        {
...
FLateGameItem LateGame::GetAssaultRifle()
{
    ...
    if (AssaultRifles.size() == 0)
    {
        if (VersionInfo.FortniteVersion > 22.40 && VersionInfo.FortniteVersion <= 26.30)
        {
            ...
            {
                AssaultRifles.push_back(FLateGameItem(1, FindObject<UFortItemDefinition>(L"/MusterCoreWeapons/Items/Weapons/MusterScopedAR/WID_Assault_MusterScoped_Athena_UC.WID_Assault_MusterScoped_Athena_UC")));
